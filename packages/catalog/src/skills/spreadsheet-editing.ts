// catalog/skills/spreadsheet-editing.ts â€” system skill, shipped with the product.
//
...
//
// `office-editing` still ships and still grants all three families â€” an agent
...
export const spreadsheetEditingSkill: SystemSkill = {
  ...
  description:
    'Create and edit Excel workbooks in the agent workspace â€” values, formulae, formatting, ' +
...
Use \`xlsx_find_cells\` to locate data before mutating it. \`xlsx_delete_rows\` and \`xlsx_delete_columns\` are irreversible â€” confirm the range with \`xlsx_read\` first and present what will be deleted before proceeding.

### Capabilities

Read with \`xlsx_read\` and \`xlsx_find_cells\`; create with \`xlsx_create\`; edit in place with full fidelity â€” values, formulae (\`=SUM(...)\`), formatting (\`xlsx_format_range\`), and structure (insert/delete rows and columns, merge, widths, freeze panes). Styles and formulae elsewhere in the workbook are preserved.
