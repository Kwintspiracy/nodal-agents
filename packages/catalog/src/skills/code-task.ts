...
3. **If you cannot delegate** (you are a worker), end your written ANSWER with a prominent \`REVIEW-REQUIRED\` marker plus the list of changed files and how to verify — that text is what your orchestrator receives, and it dispatches the review from it. \`return_result\` carries none of it. Never present write-mode work as finished without it.
...
5. **After 2 failed rounds, escalate — do not loop.** Tell the owner on their channel what was attempted and what still fails (the remaining findings), write the same thing as your answer, and finish with \`return_result\` status "blocked". Retrying past 2 rounds only burns budget and anti-loop caps.
