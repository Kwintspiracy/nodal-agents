// catalog/skills/office-editing.ts â€” system skill, shipped with the product.
//
...
// presentation-editing) now exist and are the better default â€” an agent that
...
export const officeEditingSkill: SystemSkill = {
  ...
  description:
    ...
    'Grants all three formats at once â€” if the agent only needs one, assign Spreadsheet, ' +
    'Document or Presentation editing instead and save the other formatsâ€™ context cost.',
...
### Read â†’ modify â†’ save discipline
...
3. **Confirm destructive operations.** \`xlsx_delete_rows\` and \`xlsx_delete_columns\` are irreversible â€” confirm the range with \`xlsx_read\` first and present what will be deleted before proceeding.
...
|--------|------|------------|---------------|
| Excel (.xlsx) | âœ… xlsx_read, xlsx_find_cells | âœ… xlsx_create | âœ… Full edit: values, formulae (\`=SUM(...)\`), formatting (xlsx_format_range), structure (insert/delete rows and columns, merge, widths, freeze panes). Existing styles and formulae elsewhere in the workbook are preserved. Workbooks containing charts or pivot tables are REFUSED for editing (see limits). |
| Word (.docx) | âœ… docx_read (text incl. tables) | âœ… docx_create (headings, bold/italic, bullet/numbered lists, tables, images, page breaks) | âœ… docx_append_paragraphs (faithful â€” original formatting preserved) and docx_replace_text (literal find/replace) |
| PowerPoint (.pptx) | âœ… pptx_read (per-slide text) | âœ… pptx_create (titles, bullets, body, images, tables, speaker notes, colour theme) | âœ… pptx_append_slides (existing slides untouched) and pptx_replace_text |
...
- **No pivot-table or chart support** in .xlsx (library limit): they cannot be created, and the library cannot preserve existing ones through an edit either â€” so any mutation tool REFUSES a workbook that contains charts or pivot tables (\`ok:false\` with the reason) instead of silently destroying them. Tell the user why, and offer to work on a copy without the charts, or compute the aggregation yourself and write a formatted summary sheet with formulas.
- **Text replacement is literal and run-scoped**: \`docx_replace_text\` / \`pptx_replace_text\` replace occurrences that live inside a single text run. Occurrences split across formatting runs are counted in \`skipped_fragmented\` â€” report them to the user rather than guessing.
