...
describe('architecture invariants', () => {
  ...
  it('no hardcoded Google Sheets spreadsheet URLs in source files', () => {
    assertNoViolations(
      'URL Google Sheets en dur',
      scanForPattern(
        { srcDir },
        {
          pattern: /https:\/\/docs\.google\.com\/spreadsheets\/d\/[a-zA-Z0-9_-]{10,}/,
          rule: 'gsheets-url',
        },
      ),
    );
  ...
  it('no AUTH_FAILED or hardcoded user-facing error strings', () => {
    assertNoViolations(
      ...
      scanForPattern(
        ...
        {
          pattern:
            /\[AUTH_FAILED\]|Re-authenticate in the dashboard|Google Sheets connector is not authenticated/,
