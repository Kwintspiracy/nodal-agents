...
describe('architecture invariants', () => {
  ...
  it('no hardcoded notion.so page URLs in source files', () => {
    assertNoViolations(
      'URL Notion en dur',
      scanForPattern(
        { srcDir },
        {
          pattern: /https:\/\/(www\.)?notion\.so\/[a-zA-Z0-9-]{10,}/,
          rule: 'notion-url',
        },
      ),
    );
  ...
  it('no AUTH_FAILED or hardcoded user-facing error strings', () => {
    assertNoViolations(
      ...
      scanForPattern(
        ...
        {
          pattern:
            /\[AUTH_FAILED\]|Re-authenticate in the dashboard|Notion connector is not authenticated/,
