// @nodal-agents/adapter-tavily — architecture invariant tests.
//
...
describe('architecture invariants', () => {
  ...
  it('no hardcoded Tavily API keys (tvly-...) in source files', () => {
    assertNoViolations(
      'cle Tavily en dur',
      scanForPattern(
        { srcDir },
        {
          pattern: /['"`]tvly-[A-Za-z0-9]{8,}['"`]/,
          rule: 'tavily-key',
        },
      ),
    );
  ...
  it('no AUTH_FAILED or hardcoded user-facing error strings', () => {
    assertNoViolations(
      ...
      scanForPattern(
        ...
        {
          pattern:
            /\[AUTH_FAILED\]|Re-authenticate in the dashboard|Tavily connector is not authenticated/,
