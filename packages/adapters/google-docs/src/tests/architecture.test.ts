...
describe('architecture invariants', () => {
  ...
  it('no hardcoded Google Docs document URLs in source files', () => {
    assertNoViolations(
      'URL Google Docs en dur',
      scanForPattern(
        { srcDir },
        {
  ...
  it('no AUTH_FAILED or hardcoded user-facing error strings', () => {
    assertNoViolations(
      ...
      scanForPattern(
        ...
        {
          pattern:
            /\[AUTH_FAILED\]|Re-authenticate in the dashboard|Google Docs connector is not authenticated/,
