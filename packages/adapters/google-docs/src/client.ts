...
/**
 * Create a googleapis Docs v1 instance whose access token is resolved lazily
 ...
 * that window got a spurious `docs_unauthorized` even though the underlying
 ...
 * DocsAdapterError({ code: 'docs_unauthorized' }).
 */
export function createDocsClient(getAccessToken: () => Promise<string>): docs_v1.Docs {
  const auth = new google.auth.OAuth2();
  auth.refreshHandler = async () => ({
    access_token: await getAccessToken(),
    expiry_date: Date.now() - 1,
  });
  return google.docs({ version: 'v1', auth, timeout: DEFAULT_TIMEOUT_MS });
}
