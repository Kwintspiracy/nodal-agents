// @nodalai/adapter-google-drive — architecture invariant tests
...
// Invariant 7: Official SDK only (googleapis, mammoth, pdf-parse, exceljs)
...
const srcDir = join(__filename, '..', '..'); // packages/adapters/google-drive/src
...
describe('architecture invariants', () => {
  ...
  it('no hardcoded Google Drive folder URLs in source files', async () => {
    const files = await collectSourceFiles(srcDir);
    const source = await readAll(files);

    // Real Drive URLs like https://drive.google.com/drive/folders/1AbC...
    const urlRegex = /https:\/\/drive\.google\.com\/drive\/[a-zA-Z0-9/_-]{10,}/g;
    const matches = source.match(urlRegex) ?? [];

    expect(matches, `Found hardcoded Drive URLs: ${matches.join(', ')}`).toHaveLength(0);
  });
  ...
  it('no AUTH_FAILED or hardcoded user-facing error strings', async () => {
    ...
    expect(source).not.toContain('Google Drive connector is not authenticated');
  });
