...
describe('architecture invariants', () => {
  ...
  it('no hardcoded Airtable base IDs (appXXXXXXXX) in source files', () => {
    assertNoViolations(
      'base ID Airtable en dur',
      scanForPattern(
        { srcDir },
        {
          pattern: /['"`]app[a-zA-Z0-9]{14}['"`]/,
          rule: 'airtable-base-id',
        },
      ),
    );
  });
  ...
  it('no AUTH_FAILED or hardcoded user-facing error strings', () => {
    assertNoViolations(
      ...
      scanForPattern(
        ...
        {
          pattern:
            /\[AUTH_FAILED\]|Re-authenticate in the dashboard|Airtable connector is not authenticated/,
