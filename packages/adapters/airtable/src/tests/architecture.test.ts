...
const srcDir = join(__filename, '..', '..'); // packages/adapters/airtable/src
...
describe('architecture invariants', () => {
  ...
  it('no hardcoded Airtable base IDs (appXXXXXXXX) in source files', async () => {
    const files = await collectSourceFiles(srcDir);
    const source = await readAll(files);

    // Real base IDs start with "app" followed by ~14 alphanumeric chars
    // Exclude the example in describe text ("e.g. appXXXXXXXX")
    const baseIdRegex = /['"`]app[A-Za-z0-9]{10,}['"`]/g;
    const matches = (source.match(baseIdRegex) ?? []).filter((m) => !m.includes('appXXXXXXXX'));

    expect(matches, `Found hardcoded base IDs: ${matches.join(', ')}`).toHaveLength(0);
  });
