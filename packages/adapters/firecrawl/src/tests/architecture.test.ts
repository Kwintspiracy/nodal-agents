// @nodalai/adapter-firecrawl — architecture invariant tests
// Invariant 1: No agent slugs (ender, pavel, boris, etc.) hardcoded in src/
// Invariant 6: No per-user values (UUIDs, hardcoded IDs) in src/
...
const srcDir = join(__filename, '..', '..'); // packages/adapters/firecrawl/src
...
describe('architecture invariants', () => {
  ...
  it('no AUTH_FAILED or hardcoded user-facing error strings', async () => {
    ...
    expect(source).not.toContain('Firecrawl connector is not authenticated');
  });
