...
describe('architecture invariants', () => {
  ...
  it('no hardcoded Gmail URLs in source files', () => {
    assertNoViolations(
      'URL Gmail en dur',
      scanForPattern(
        { srcDir },
        {
          pattern: /https:\/\/mail\.google\.com\/mail\/[a-zA-Z0-9/#_?=&-]{10,}/,
          rule: 'gmail-url',
        },
      ),
    );
  ...
  it('no AUTH_FAILED or hardcoded user-facing error strings', () => {
    assertNoViolations(
      ...
      scanForPattern(
        ...
        {
          pattern:
            /\[AUTH_FAILED\]|Re-authenticate in the dashboard|Gmail connector is not authenticated/,
