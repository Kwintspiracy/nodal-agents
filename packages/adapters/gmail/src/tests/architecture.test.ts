// @nodal-agents/adapter-gmail — architecture invariant tests
...
const srcDir = join(__filename, '..', '..'); // packages/adapters/gmail/src
...
describe('architecture invariants', () => {
  ...
  it('no hardcoded Gmail URLs in source files', async () => {
    const files = await collectSourceFiles(srcDir);
    const source = await readAll(files);

    // Real Gmail URLs
    const urlRegex = /https:\/\/mail\.google\.com\/mail\/[a-zA-Z0-9/#_?=&-]{10,}/g;
    const matches = source.match(urlRegex) ?? [];

    expect(matches, `Found hardcoded Gmail URLs: ${matches.join(', ')}`).toHaveLength(0);
  });
  ...
  it('no AUTH_FAILED or hardcoded user-facing error strings', async () => {
    ...
    expect(source).not.toContain('Gmail connector is not authenticated');
  });
