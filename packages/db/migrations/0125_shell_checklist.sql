...
--> statement-breakpoint
-- Un agent à qui le propriétaire a déjà donné le shell sans demander (le
-- toggle Yolo : une règle `run_command → auto_approve`) garde ce qu'il avait :
-- chaque sorte d'action y est permise… sauf sortir de ses dossiers, la frontière
-- que ce ticket pose. Tous les autres agents demandent pour tout (NULL).
UPDATE "agents"
