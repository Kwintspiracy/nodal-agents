...
export const fileWriteTool: ToolDefinition<typeof FileWriteInputSchema, FileWriteOutput> = {
  ...
  execute: async (input, ctx) => {
    try {
      ...
      //
      // Le type n'est PAS reclassé ici : c'est la décision du hook, relue
      // telle quelle. Elle était refaite, et l'écart entre les deux réponses
      // rendait l'état introuvable — que l'écart vienne du disque (le premier
      // `package.json` d'un dossier, revue Codex post-merge de la PR #66,
      // constat C3) ou de la table `code_projects`, qui peut changer entre le
      // hook et ici (revue C, dette #88). Un seul classement, donc aucun écart
      // possible. Le repli n'existe que hors du seam, où aucune intention n'a
      // été posée — voir `declaredWrittenFileType`.
