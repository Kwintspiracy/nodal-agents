...
describe('parent receives a typed delegation record @cap:organiser-equipe/moteur', () => {
  ...
  // ajouté sans que personne l'ait décidé. `review_verdict` est le seul toléré
  // en plus : il arrive par la PR #170, et cette branche peut être à jour ou non.
  const CLES_DU_CONTRAT = ['error', 'exit_reason', 'hint', 'status', 'summary', 'tools_used'];

  function clesDuContrat(payload: Record<string, unknown>): string[] {
    return Object.keys(payload)
      .filter((k) => k !== 'review_verdict')
      .sort();
  }
