// job-microsoft-oauth-refresh.test.ts — audit#2 M-12-equivalent regression for Outlook Mail
//
// Mirrors job-google-oauth-refresh.test.ts for the outlook-mail adapter.
// Before a fix analogous to M-12, execute.ts would resolve a Microsoft OAuth
// connector's access token ONCE at job setup and bake that static string into
// the adapter's tools for the tool's entire lifetime via entry.toolFactory().
// A job whose Microsoft token expired mid-run (Graph delegated tokens live
// ~60-90min; IDLE_RESET is 4h) would get a permanent outlook_unauthorized
// even though the DB credential was still valid and refreshable.
//
// This test proves the fix at the seam that matters: when the LLM calls a
// microsoft-oauth-backed tool MULTIPLE times in the same job, the credential
// is re-read from the DB before each underlying network attempt — not
// captured once. We assert this by counting invocations of
// getDecryptedCredentialById (spied via a partial module mock), which is
// exactly what the resolver execute.ts passes to entry.toolFactoryWithResolver
// on every resolution (see packages/runner-adapters/src/registry.ts and
...
vi.mock('@nodal-agents/llm', async (importOriginal) => {
  ...
  return {
    ...
    createLlmClient: (..._args: Parameters<typeof actual.createLlmClient>) => {
      ...
      if (!active) throw new Error('job-microsoft-oauth-refresh.test: no active LLM client set');
      return active;
    },
  };
});
...
describe('job-microsoft-oauth-refresh: token resolved per-call, not once at setup', () => {
  it('getDecryptedCredentialById is invoked again for a second outlook_list_messages call in the same job', async () => {
    await db
    ...
    const [job] = await db
      ...
      .values({
        ...
        agentId: seed.agentId,
        channel: 'api',
        task: 'List Outlook messages twice',
        status: 'pending',
        messages: [],
        chainCount: 0,
      })
    ...
    // Two separate turns each calling outlook_list_messages, before return_result.
    // The underlying Graph SDK HTTP call will fail in this harness (no real
    // network) — that's fine, we only assert on how many times the
    // credential resolver ran, not on the tool's result.
    const client = makeMockLlmClient([
      {
        toolCalls: [
          { toolCallId: 'tc-1', toolName: 'outlook_list_messages', args: { max_results: 5 } },
        ],
      },
      {
        toolCalls: [
          { toolCallId: 'tc-2', toolName: 'outlook_list_messages', args: { max_results: 5 } },
        ],
      },
      {
        toolCalls: [
          {
            ...
            args: { status: 'blocked', reason: 'Outlook call unresolved under this harness' },
          },
        ],
      },
    ]);
    ...
    // Before a fix analogous to M-12, execute.ts would resolve the token
    // exactly ONCE per job regardless of how many times the tool was called
    // — this assertion would have failed (stuck at 1).
