...
describe.skipIf(!process.env['LIVE_SKILL_INSTALL'])('LIVE install: zarazhangrui/frontend-slides', () => {
  it('installs from GitHub, writes the row, detects scripts, lands files', async () => {
    ...
    await expect(
      stat(join(store, 'frontend-slides', 'bold-template-pack', 'templates', 'blue-professional', 'design.md')),
    ).resolves.toBeTruthy();
  }, 120000);
});

describe.skipIf(!process.env['LIVE_SKILL_INSTALL'])('LIVE install: clawhub image-studio (zip)', () => {
