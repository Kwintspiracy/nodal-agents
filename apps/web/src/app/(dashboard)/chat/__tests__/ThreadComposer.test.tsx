...
// haut de deux lignes, sur sa propre surface, depuis #135.
//
...
describe('ThreadComposer', () => {
  it('est une zone de texte de deux lignes à vide, qui nomme l’agent', async () => {
  ...
  it('porte sa propre surface et son texte de 14 px (#135)', async () => {
    ...
    expect(el.className).toContain('min-h-[40px]');
  ...
  it('après un envoi, la zone vidée garde ses deux lignes', async () => {
    await render(<ThreadComposer conversationId="conv-1" agentName="Alfred" />);
    ...
    // Le chiffre est écrit ici EN DUR — deux lignes de 20 px : lu depuis les
