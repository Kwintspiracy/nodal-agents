    ...
    <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
      {messages.map((msg, i) => {
        ...
        return (
          ...
          >
            <span
              className={`inline-block text-label-11 uppercase tracking-wider mb-1.5 ${
              ...
              {blocks.map((block, j) => {
                ...
                if (block.kind === 'tool-call') {
                  return (
                    <div key={j} className="border-l-2 border-run/30 pl-3 py-1 bg-run-bg rounded-r">
                      <p className="text-label-11 uppercase tracking-wider text-run mb-1">
                        → call {block.toolName}
                      </p>
                      <pre className="text-mono-12 text-ink-2 whitespace-pre-wrap break-words">
                ...
                return (
                  <div key={j} className="border-l-2 border-ok/30 pl-3 py-1 bg-ok-bg rounded-r">
                    <p className="text-label-11 uppercase tracking-wider text-ok mb-1">
