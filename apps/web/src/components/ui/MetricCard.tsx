...
export default function MetricCard({ label, value, unit, subtle, delta }: Props) {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-1 rounded-xl border border-rule-2 bg-paper px-3.5 py-3">
      ...
      {subtle && <div className="text-body-12 leading-none! text-ink-3">{subtle}</div>}
      {delta && (
        <div
          className={`mt-0.5 inline-flex items-center gap-1 text-mono-12 leading-none! tracking-[0.02em] ${deltaColour}`}
        >
