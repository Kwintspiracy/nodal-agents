...
export default function AvatarStack({ avatars, max = 5, label, className = '' }: Props) {
  if (avatars.length === 0) {
    return label ? (
      <span className={`font-sans text-[12.5px] leading-none text-ink-3 ${className}`}>
        {label}
      </span>
    ) : null;
  }
    ...
    <span className={`inline-flex items-center gap-2.5 ${className}`}>
      <span className="inline-flex">
        {head.map((a, i) => (
          <span
            key={a.id}
            className={`flex h-6 w-6 items-center justify-center overflow-hidden rounded-full border-2 border-paper bg-agent-vivid text-[9.5px] font-semibold leading-none tracking-[0.04em] text-[#0a0a0a] ${
              i === 0 ? '' : '-ml-[7px]'
            }`}
            title={a.name}
          >
        ...
        {overflow > 0 && (
          <span className="flex h-6 w-6 -ml-[7px] items-center justify-center rounded-full border-2 border-paper bg-hover text-[9.5px] font-semibold leading-none text-ink-3">
            +{overflow}
          </span>
        )}
      </span>
      {label && <span className="font-sans text-[12.5px] leading-none text-ink-3">{label}</span>}
    </span>
  );
}
