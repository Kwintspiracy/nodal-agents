      ...
      {items.map((it) => {
        ...
        return (
          <button
            ...
            className={`inline-flex h-8 items-center gap-1.5 rounded-[9px] border px-3.5 text-[12.5px] font-medium leading-none transition-colors ${
              isActive
          ...
          >
            ...
            {it.count !== undefined && (
              <span
                className={`font-mono text-[11px] leading-none ${isActive ? 'opacity-65' : 'opacity-55'}`}
              >
                {it.count}
              </span>
            )}
