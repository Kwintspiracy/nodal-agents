...
describe('un fil que personne n’a nommé @cap:reprendre-conversation/moteur', () => {
  ...
  it('s’appelle « Untitled », et le nom vient du chemin PARTAGÉ', async () => {
    // Le repli vivait chez chaque appelant ; il vit maintenant dans le chemin
    // de nommage partagé (`nommerLesFils`), que toute lecture de la barre
    // traverse. Un fil que personne n'a nommé porte donc le même nom d'une
    // lecture à l'autre, par construction.
    //
    // Mutation vérifiée : le `=== '' ? 'Untitled'` retiré de `nommerLesFils`
    // → ce test rougit, la lecture rend une chaîne vide.
    const { listFolderThreadReadsAction } = await import('../conversation-actions.ts');
