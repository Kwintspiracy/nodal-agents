...
// conversations arrivent — un par canal branché, « Nodal chats » pour celles
// ouvertes ici, « Routines » pour les runs programmés.
//
...
/** Le dossier des runs programmés. Il mène à `/scheduled`, pas à `/chat`. */
...
export function folderOfJobChannel(channel: string | null): string | null {
  ...
  if (channel === 'cron') return ROUTINES_FOLDER;
...
export function folderLabel(key: string): string {
  ...
  if (key === ROUTINES_FOLDER) return 'Routines';
  return CHANNEL_LABELS[key] ?? key;
}

/** Où mène un dossier. Les routines ont leur propre page ; tout le reste
 *  filtre `/chat`. */
export function folderHref(key: string): string {
  if (key === ROUTINES_FOLDER) return '/scheduled';
...
export type ChatFoldersInput = {
  ...
  /** Le chemin courant, pour l'état actif de « Routines ». */
  pathname: string;
 ...
 * (invariant #4).
 *
 * « Nodal chats » et « Routines » sont TOUJOURS là : ce sont les deux
 * destinations permanentes du produit — la page existe, vide ou non — et les
 * faire apparaître et disparaître ferait bouger le menu sous le curseur.
 */
function existingKeys(input: Pick<ChatFoldersInput, 'channels' | 'waiting' | 'running'>): string[] {
  ...
  for (const c of input.channels) {
    ...
    if (key !== null && key !== DASHBOARD_FOLDER && key !== ROUTINES_FOLDER) chan.add(key);
  }
  ...
  for (const [key, count] of Object.entries(input.running)) {
    if (count > 0 && key !== DASHBOARD_FOLDER && key !== ROUTINES_FOLDER) chan.add(key);
  }
  ...
  return [...known, ...unknown, DASHBOARD_FOLDER, ROUTINES_FOLDER];
}
...
/**
 * Un dossier est ACTIF quand l'URL le désigne : le `folder=` de `/chat` pour
 * les dossiers de chat, le chemin lui-même pour les routines.
 *
...
function isFolderActive(key: string, pathname: string, folderParam: string | null): boolean {
  if (key === ROUTINES_FOLDER) {
    return pathname === '/scheduled' || pathname.startsWith('/scheduled/');
  }
