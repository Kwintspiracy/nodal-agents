...
/**
 * Recent jobs across the workspace, joined with the agent that ran them.
 * Powers the "This week's activity" table on Home — a tight, just-the-
 * meaningful-columns view of what the fleet has been up to.
 *
 * Distinct from listJobsAction because Home renders the agent inline
 * (name, slug, avatar) — joining at the DB level is cheaper than fanning
 * out an extra query per row.
 */
export async function getHomeActivityAction(
  opts: { limit?: number } = {},
): Promise<ActionResult<HomeActivityRow[]>> {
  try {
    const session = await getSession();
    ...
    return ok(
      rows.map((r) => ({
        jobId: r.jobId,
        agentId: r.agentId,
        // Fall back when the join misses (deleted agent) so the row still
        // renders — better than dropping recent activity silently.
    ...
    console.error('[getHomeActivityAction]', err);
    return fail('db_error', 'Failed to load recent activity');
  }
}
