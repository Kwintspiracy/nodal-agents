...
**Cause.** A previous run was closed without `Ctrl+C` (or without
`nodal-agents down`), leaving orphan processes holding the ports — or the OS has
reserved a port (Windows Hyper-V/WinNAT excluded ranges). On startup the CLI
sweeps orphans on the configured ports and, if a port is still unbindable,
