...
It attempts a real write with the arguments Nodal passes. Exit 0 means confined,
1 means not, 2 means it could not tell — and it never reports "confined" without
positive proof that a command was attempted and refused.
...
**Symptom.** The install ended with `npm warn allow-scripts`, and `nodal-agents
up` stops straight away on a line naming `@embedded-postgres/<platform>`.
...
**Fix.** Approve that one package and install again. The name is the one npm
printed:
...
The other packages npm lists (`@whiskeysockets/baileys`, `protobufjs`,
