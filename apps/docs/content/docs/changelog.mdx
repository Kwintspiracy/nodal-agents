...
- **Conversations that hold the thread.** Session memory measures a pause from when the reply was *delivered*, not when the job started — so a slow task no longer makes a quick follow-up look like an hour of silence. The idle reset widened (30 min → 4 h), the history budget grew, the reply's tail is kept, and a job that failed but spoke to you stays in the thread. Measured amnesia on a real 122-message conversation: 39% → 13%.
