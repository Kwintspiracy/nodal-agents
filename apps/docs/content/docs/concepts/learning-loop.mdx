...
4. The job is substantial enough — it ran at least `REFLECTION_MIN_TURNS` turns
   (default: 3). Short heartbeat or cron runs are not worth reflecting on. (An
   optional `REFLECTION_MIN_TOOL_ITERS` tool-iteration gate can tighten this
   further; it falls back to a constant when unset.)
...
| `REFLECTION_MIN_TURNS`    | `3`       | Minimum job turns before a job is "substantial" enough to reflect on.                 |
...
(Two optional gates — `REFLECTION_MIN_TOOL_ITERS` and `REFLECTION_MAX_NEW_SKILLS_PER_PASS` — have no env default and fall back to internal constants when unset.)
