...
the workspace's root agent (or a chosen agent by slug). Jobs land on the Runs
page with channel `mcp`, run under the agent's own approval rules and budgets,
and can never use the configuration tools. When the client disconnects, the
