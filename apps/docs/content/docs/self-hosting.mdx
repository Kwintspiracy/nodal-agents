...
You can enable email+password auth on a loopback install without switching to full LAN mode. In `~/.nodalai/config.json`:
...
Restart for the change to take effect. The dashboard shows a login page at `/login`.
