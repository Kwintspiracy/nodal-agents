...
<Callout type="warn">
  Bearer-token mode is **not yet supported end-to-end**. The runner API honors `Authorization:
  Bearer …`, but the dashboard's bearer-token auth provider is still a scaffold that would fall back
