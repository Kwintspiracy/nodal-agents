...
For programmatic or headless access, set `AUTH_MODE=bearer-token` and provide the token. The token lives at the top level of `config.json` as `bearerToken`:

```json
{
  "bind": "loopback",
  "bearerToken": "your-long-random-secret"
}
```

Every request to the runner API must then carry `Authorization: Bearer your-long-random-secret`. Restart for the change to take effect.
