...
A grant says what the **workspace** allows. A separate per-agent setting, **`may_change_team`**, says whether _that_ agent may rearrange its own team, and it is **off by default** — including for the ROOT, and including for every agent that already existed when the setting shipped.
